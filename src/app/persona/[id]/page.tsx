...
export default function PersonPage({ params }: PersonPageProps) {
    ...
    const fetchPersonData = async () => {
      try {
        ...
        const [personData, creditsData, imagesData] = await Promise.all([
          getPersonDetails(resolvedParams.id),
          getPersonCredits(resolvedParams.id),
          getPersonImages(resolvedParams.id)
        ]);
  ...
  }, [resolvedParams.id, showSuccess, showError, isInitialLoad]);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const fileName = `${person?.name || 'person'}_${currentImageIndex + 1}.${extension}`;
      ...
      showSuccess('Imagen descargada', 'La imagen se descargó correctamente');
    } catch (error) {
      console.error('Error al descargar la imagen:', error);
      showError('Error al descargar', 'No se pudo descargar la imagen');
    }
  };
