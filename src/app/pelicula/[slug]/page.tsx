...
export default function MoviePage({ params }: MoviePageProps) {
  ...
  // Extraer el ID de la película del slug usando la nueva utilidad
  ...
  useEffect(() => {
    const fetchMovieData = async () => {
      try {
        ...
        const [movieData, creditsData, videosData, similarData, recommendedData] = await Promise.all([
          ...
          getMovieVideos(movieId.toString()),
          getSimilarMovies(movieId.toString()),
          getRecommendedMovies(movieId.toString())
        ]);
    ...
    if (movieId && isValidId) {
      fetchMovieData();
    } else if (!isValidId) {
      setError('URL de película inválida');
      setLoading(false);
    }
  }, [movieId, isValidId, showSuccess, showError, isInitialLoad]);
