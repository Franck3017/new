...
import { ROUTES, generateGenreUrl } from '@/utils/urlHelpers';
...
export default function SeriesPage() {
  ...
  const [filteredGenres, setFilteredGenres] = useState(() => [...TV_GENRES]);
  ...
  const [filterStats, setFilterStats] = useState<{
    ...
    genres: string[];
    years: number[];
  }>({ totalShows: 0, averageRating: 0, genres: [], years: [] });
  const [filterHistory, setFilterHistory] = useState<string[]>([]);
  const [isFilterExpanded, setIsFilterExpanded] = useState(false);

  // Función para eliminar duplicados y procesar datos de series
  const removeDuplicates = (shows: Movie[]): Movie[] => {
    const seen = new Set();
    return shows.filter(show => {
      ...
      ...show,
      media_type: 'tv' // Agregar el tipo de medio para que MovieCard sepa que es una serie
    }));
  };
  ...
  const handleQuickFilter = async (filter: string) => {
    setQuickFilters(prev => {
      ...
      if (newFilters.length === 0) {
        ...
        setFilteredContent([]);
        setFilterStats({ totalShows: 0, averageRating: 0, genres: [], years: [] });
        return newFilters;
      }
  ...
  const calculateFilterStats = (shows: Movie[]) => {
    if (shows.length === 0) {
      return { totalShows: 0, averageRating: 0, genres: [], years: [] };
    }

    const ratings = shows.map(show => show.vote_average || 0).filter(rating => rating > 0);
    const averageRating = ratings.length > 0 ? ratings.reduce((a, b) => a + b, 0) / ratings.length : 0;
    
    // Usar genre_ids si existe, sino usar un array vacío
    const genres = Array.from(new Set(shows.flatMap(show => (show as any).genre_ids || [])));
    const years = Array.from(new Set(shows.map(show => {
    ...
    return {
      ...
      genres: genres.map(id => TV_GENRES.find(g => g.id === id)?.name || '').filter(name => name),
  ...
  const loadFilteredContent = async (filters: string[]) => {
    ...
    try {
      ...
      for (const filter of filters) {
        ...
        switch (filter) {
          case 'live':
            response = await getOnAirTVShows(1);
            break;
          case 'rated':
        ...
        if (response?.results) {
          const shows = response.results.map((show: any) => ({
            ...show,
            media_type: 'tv'
          }));
          allShows.push(...shows);
      ...
      // Eliminar duplicados y limitar a 30 series para más opciones
      const uniqueShows = removeDuplicates(allShows).slice(0, 30);
      
      // Ordenar contenido según el criterio seleccionado
      const sortedShows = sortContent(uniqueShows, sortBy);
      
      // Calcular estadísticas
      const stats = calculateFilterStats(sortedShows);
      
      setFilteredContent(sortedShows);
  ...
  // Función para cargar series por género
  const fetchTVShowsByGenre = async (genreId: number) => {
    ...
    try {
      const response = await getTVShowsByGenre(genreId.toString(), 1);
      const shows = removeDuplicates(response.results || []);
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      const initialGenres = TV_GENRES.slice(0, 8);
      const promises = initialGenres.map(genre => fetchTVShowsByGenre(genre.id));
      await Promise.all(promises);

      // Cargar géneros restantes en segundo plano
      setTimeout(async () => {
        const remainingGenres = TV_GENRES.slice(8);
        ...
        for (let i = 0; i < remainingGenres.length; i += 3) {
          ...
          const batchPromises = batch.map(genre => fetchTVShowsByGenre(genre.id));
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (searchQuery.trim() === '') {
        setFilteredGenres(() => [...TV_GENRES]);
        return;
      }

      const searchTerm = searchQuery.toLowerCase();
      const filtered = TV_GENRES.filter(genre => 
        genre.name.toLowerCase().includes(searchTerm) ||
        // También buscar en las series de cada género si están cargadas
        (genreData[genre.id] && genreData[genre.id].some(show => 
          (show.title || show.name || '').toLowerCase().includes(searchTerm)
        ))
      );
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 sm:mb-8">
              ...
              <div className="flex sm:hidden lg:flex items-center gap-2 sm:gap-4">
                <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 sm:py-2 bg-emerald-500/10 rounded-lg border border-emerald-500/20">
                  <FiTv className="h-3 w-3 sm:h-4 sm:w-4 text-emerald-400" />
                  <span className="text-xs sm:text-sm text-white font-medium">{TV_GENRES.length}</span>
                </div>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sm:gap-6 mb-6 sm:mb-8">
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="p-2 sm:p-3 bg-gradient-to-br from-emerald-500/20 to-blue-500/20 rounded-xl border border-white/10">
                  <FiTv className="h-6 w-6 sm:h-8 sm:w-8 text-emerald-400" />
                </div>
                <div>
                  <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-1">Series de TV</h1>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
                ...
                <div className="relative">
                  <input
                    type="text"
                    placeholder="Buscar géneros o series..."
                    value={searchQuery}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
              <div className="lg:col-span-2">
                <p className="text-gray-300 leading-relaxed text-sm sm:text-base">
                  Descubre series de televisión organizadas por géneros. Explora contenido premium, 
                  encuentra nuevas series y mantente al día con las últimas tendencias de la televisión.
                </p>
              </div>
              ...
              <div className="flex items-center justify-start lg:justify-end gap-3 sm:gap-4">
                <div className="text-center">
                  <div className="text-lg sm:text-2xl font-bold text-white">{TV_GENRES.length}</div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-xs text-gray-400 uppercase tracking-wide">Series</div>
                </div>
              </div>
            </div>
                        ...
                        {filterHistory.slice(0, 3).map((filter) => (
                          ...
                          >
                            {filter === 'live' ? 'En Vivo' : 
                             ...
                             filter === 'trending' ? 'Tendencias' : 'Nuevas'}
                          </button>
                        ))}
                      </div>
                    </div>
          ...
          {showFilteredContent && (
            <div className="mb-8">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-white">
                      {quickFilters.length === 1 
                        ? (quickFilters[0] === 'live' ? 'Series en Vivo' :
                           quickFilters[0] === 'rated' ? 'Series Mejor Valoradas' :
                           quickFilters[0] === 'trending' ? 'Series en Tendencia' :
                           'Series Nuevas')
                        : 'Series Filtradas'
                      }
                    </h2>
                    <p className="text-gray-400 text-sm">
                      {filterStats.totalShows} series • Calificación promedio: {filterStats.averageRating}/10
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="px-3 py-1 bg-gray-700/50 rounded-lg border border-gray-600/50">
                    <span className="text-white text-sm font-medium">
                      {filteredContent.length} de {filterStats.totalShows}
                    </span>
                  </div>
              ...
              {filteredContentLoading ? (
                ...
                <HorizontalScroll>
                  {filteredContent.map((show, index) => (
                    <div key={`filtered-${show.id}-${index}`} className="flex-shrink-0 w-48 sm:w-56 md:w-64 lg:w-72">
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold text-white mb-2">No se encontraron series</h3>
                  <p className="text-gray-400 mb-4">
                    No hay series disponibles con los filtros seleccionados.
                  </p>
              ...
              {filteredGenres.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-gray-400 mb-6">
                    {searchQuery ? (
                      <>
                        No se encontraron géneros o series que coincidan con &quot;{searchQuery}&quot;.
                        <br />
                  ...
                  {searchQuery && (
                    <div className="flex items-center justify-center gap-4">
                      ...
                      <button 
                        onClick={() => setFilteredGenres(() => [...TV_GENRES])}
                ...
                filteredGenres.map((genre) => {
                  ...
                  const shows = genreData[genre.id];
                      ...
                      {isLoading ? (
                        ...
                        <HorizontalScroll>
                          {shows.map((show, index) => (
                            <div key={`${genre.id}-${show.id}-${index}`} className="flex-shrink-0 w-48 sm:w-56 md:w-64 lg:w-72">
                              <MovieCard movie={show} mediaType="tv" />
                            </div>
                          ))}
                        </HorizontalScroll>
                        ...
                        <div className="text-center py-12">
                          <FiTv className="h-12 w-12 text-gray-500 mx-auto mb-4" />
                          <p className="text-gray-400">No hay series disponibles en este género</p>
                        </div>
                      )}
                    </section>
                  );
